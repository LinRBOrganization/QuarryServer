...
namespace LB.Web.DB.BLL
{
    public class BLLDbCameraConfig : IBLLFunction
    {
        ...
        public void InsertAndUpdate(FactoryArgs args,
           ...
           t_String IPAddress1, t_ID Port1, t_String Account1, t_String Password1,
           t_String IPAddress2, t_ID Port2, t_String Account2, t_String Password2,
           t_String IPAddress3, t_ID Port3, t_String Account3, t_String Password3,
           t_String IPAddress4, t_ID Port4, t_String Account4, t_String Password4)
        {
            using(DataTable dtExists= _DALDbCameraConfig.VerifyExists(args, MachineName))
            {
                if (dtExists.Rows.Count == 0)
                {
                    _DALDbCameraConfig.Insert(args, out CameraConfigID, MachineName, IPAddress1, Port1, Account1, Password1,
                        IPAddress2, Port2, Account2, Password2, IPAddress3, Port3, Account3, Password3,
                        IPAddress4, Port4, Account4, Password4);
                }
                else
                {
                    CameraConfigID = new t_BigID(dtExists.Rows[0]["CameraConfigID"]);
                    _DALDbCameraConfig.Update(args, CameraConfigID, MachineName, IPAddress1, Port1, Account1, Password1,
                        IPAddress2, Port2, Account2, Password2, IPAddress3, Port3, Account3, Password3,
                        IPAddress4, Port4, Account4, Password4);
                }
            }
